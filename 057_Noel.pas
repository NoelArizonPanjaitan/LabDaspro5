...
begin
  case grade of
    ...
    'E': convertToReal := nilaiE;
      ...
      begin
        writeln('Error: Nilai tidak valid - ', grade, '. Nilai diabaikan.');
        convertToReal := 0.0;
      end;
  end;
end;
...
begin
  ...
  begin
    ...
    repeat
      ...
      if matkul[i].sks < 0 then
        writeln('Error: SKS Tidak Boleh negatif. Mohon Masukkan Angka Yang Valid.');
  ...
  begin
    repeat
      ...
      if convertToReal(mahasiswa.nilaiString[i]) = 0.0 then
        WriteLn('Error: Nilai Tidak Valid. Masukkan nilai ', matkul[i].nama, ' : ');
    until convertToReal(mahasiswa.nilaiString[i]) <> 0.0;
  end;
